...
async def generate_visual_notes(request: TextRequest, user_data: Dict = Depends(get_current_user_doc)):
    uid = user_data['uid']
    ...
    try:
        ...
        for part in response.parts:
            if part.inline_data:
                ...
                db.collection('users').document(uid).update({"tokens_remaining": firestore.Increment(-1)})
...
async def submit_feedback(feedback: FeedbackRequest, user_data: Dict = Depends(get_current_user_doc)):
...
{feedback.message}
    ...
    payload = {"title": issue_title, "body": issue_body, "labels": ["user-feedback"]}
